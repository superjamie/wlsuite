...
static void writeAnimation(char *outputDir, wlPicsAnimation animation)
{
	...
	fprintf(htmlFile,
		"    <div style=\"position:relative;width:96px;height:84px\">\n");

	// Determine the filename format for all written files of this animation
	sprintf(format, "%%0%ii.%%s",
		(int)log10(animation->instructions->quantity) + 1);

	// Write the base frame
	sprintf(filename, format, 0, "png");
	fprintf(htmlFile,
		"      <img src=\"%s\" style=\"position:absolute;width:100%%;height:100%%\" />\n",
		filename);
	...
	for (i = 0; i < animation->instructions->quantity; i++) {
		...
		file = fopen(filename, "wb");
		if (!file)
			die("Unable to write animation layer to %s: %s\n",
			    filename, strerror(errno));
		gdImageGifAnimBegin(transpImage, file, 1, 0);
		gdImageGifAnimAdd(transpImage, file, 0, 0, 0,
				  set->instructions[0]->delay * 6,
				  gdDisposalNone, NULL);
		prevImage = NULL;
		for (j = 0; j < set->quantity - 1; j++) {
			...
			// simply ignore it
			if (animation->updates->sets[instruction->update]
				    ->quantity == 0)
				continue;

			wlAnimationApply(
				frame,
				animation->updates->sets[instruction->update]);
			frameImage = createImage(frame);
			gdImageGifAnimAdd(frameImage, file, 0, 0, 0,
					  set->instructions[j + 1]->delay * 6,
					  gdDisposalNone,
					  prevImage ? prevImage : baseImage);
			if (prevImage)
