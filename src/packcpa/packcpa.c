...
static wlCpaAnimation *readAnimation(char *inputDir)
{
	...
	if (!delays)
		die("Unable to read delays.txt file: %s\n", strerror(errno));
	...
	for (i = 1; i < quantity; i++) {
		...
		while (1) {
			if (!fgets(buffer, sizeof(buffer), delays)) {
				die("Unable to read data from delay.txt: ",
				    strerror(errno));
				return NULL;
			}
			line = strCreate();
		...
		frame = i == quantity - 1 ? lastFrame : readImage(filenames[i]);
		wlCpaAddFrame(animation, frame, baseFrame,
			      i == 11 ? lastFrame : NULL, delay);
		free(baseFrame);
		baseFrame = frame;
	}
	free(baseFrame);
