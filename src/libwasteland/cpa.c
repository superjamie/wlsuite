...
void wlCpaFree(wlCpaAnimation *animation)
{
	...
	for (frame = 0; frame < animation->quantity; frame++) {
		for (update = 0; update < animation->frames[frame]->quantity;
		     update++) {
...
wlCpaAnimation *wlCpaReadStream(FILE *stream)
{
	...
	while (1) {
		...
		// animation data
		delay = wlHuffmanReadWord(stream, rootNode, &dataByte,
					  &dataMask);
		if (delay == -1) {
			wlHuffmanFreeNode(rootNode);
			wlCpaFree(animation);
		...
		frame->updates = NULL;
		animation->quantity++;
		animation->frames = (wlCpaFrame **)realloc(
			animation->frames,
		...
		while (1) {
			offset = wlHuffmanReadWord(stream, rootNode, &dataByte,
						   &dataMask);
			if (offset == -1) {
				wlHuffmanFreeNode(rootNode);
				wlCpaFree(animation);
			...
			for (x = 0; x < 8; x += 2) {
				b = wlHuffmanReadByte(stream, rootNode,
						      &dataByte, &dataMask);
				if (b == -1) {
					wlHuffmanFreeNode(rootNode);
					wlCpaFree(animation);
			...
			frame->quantity++;
			frame->updates = (wlCpaUpdate **)realloc(
				frame->updates,
...
int wlCpaWriteStream(wlCpaAnimation *animation, FILE *stream)
{
	...
	if (!wlWriteDWord(288 * 128 / 2, stream))
		return 0;

	// Write the MSQ header
	if (fprintf(stream, "msq") != 3)
		return 0;
	if (fputc(0, stream) == EOF)
		return 0;

	// Encode the pixels of the base frame
	memcpy(encodedPixels, animation->baseFrame,
	       sizeof(wlPixel) * 288 * 128);
	...
	dataByte = 0;
	dataMask = 0;
	if (!wlHuffmanWriteNode(rootNode, stream, &dataByte, &dataMask))
		return 0;
	...
	if (!wlFillByte(0, stream, &dataByte, &dataMask))
		return 0;
	...
	// Write the uncompressed picture size
	if (!wlWriteDWord(size, stream))
		return 0;

	// Write the animation data header
	if (fputc(0x08, stream) == EOF)
		return 0;
	if (fputc(0x67, stream) == EOF)
		return 0;
	if (fputc(0x01, stream) == EOF)
		return 0;
	if (fputc(0x00, stream) == EOF)
		return 0;
		   ...
		   wlImage prevFrame, wlImage lastFrame, int delay)
{
	...
	frame->updates = NULL;
	animation->quantity++;
	animation->frames = realloc(animation->frames,
				    sizeof(wlCpaFrame *) * animation->quantity);
	animation->frames[animation->quantity - 1] = frame;
		...
		for (x = 0; x < w; x += 8) {
			...
			for (i = 0; i < 8; i++) {
				if (prevFrame->pixels[y * w + x + i] !=
					...
					changed = 1;
				if (lastFrame &&
				    lastFrame->pixels[y * w + x + i] !=
					    curFrame->pixels[y * w + x + i])
					changed = 1;
			}
			if (changed) {
				update = (wlCpaUpdate *)malloc(
					sizeof(wlCpaUpdate));
				update->x = x;
				update->y = y;
				memcpy(update->pixels, &curFrame[y * 288 + x],
				       8);
				frame->quantity++;
				frame->updates =
					realloc(frame->updates,
						sizeof(wlCpaUpdate *) *
							frame->quantity);
				frame->updates[frame->quantity - 1] = update;
			}
